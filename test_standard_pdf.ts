...
async function testStandardPDFGeneration() {
  try {
    // Test PRD generation with standard format
    const prdContent = `
## 1. Visão Geral
...
`;
    ...
    // Test Tasks generation with standard format
    const tasksContent = `
## 1. Project Overview
...
`;
    ...
    const prdPath = path.join(documentsDir, 'standard_prd.pdf');
    const tasksPath = path.join(documentsDir, 'standard_tasks.pdf');

    fs.writeFileSync(prdPath, prdPdf);
    fs.writeFileSync(tasksPath, tasksPdf);

    console.log(`Standard PRD generated at: ${prdPath}`);
    console.log(`Standard Tasks generated at: ${tasksPath}`);
    console.log('Standard PDF generation test completed successfully!');
  } catch (error) {
    console.error('Error during standard PDF generation test:', error);
  }
}

testStandardPDFGeneration().then(() => {
